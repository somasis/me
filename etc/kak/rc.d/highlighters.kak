...
hook global -group user-highlight-long-lines NormalIdle .* %{
    add-highlighter -override buffer/user-highlight-long-lines \
        dynregex "^[^\n]{%opt{autowrap_column}}([^\n]+)$" 1:red+bf
}

# Highlight the current line and column the cursor is on.
# set-face global UserCurrent default,rgb:2b2b2b

# hook global -group user-highlight-cursor RawKey .* %{
#     add-highlighter -override window/user-highlight-cursor-line \
#         line %val{cursor_line} UserCurrent
#     add-highlighter -override window/user-highlight-cursor-column \
